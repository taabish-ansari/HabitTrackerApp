    ...
    <main className="auth-shell">
      <section className="auth-card">
        ...
        <div className="auth-heading">
          ...
          <h1>{mode === 'login' ? 'Welcome back.' : 'Start a better routine.'}</h1>
          <p className="muted">Small wins, tracked clearly.</p>
        </div>
...
function Dashboard({ user }) {
  ...
  const loadGame = async () => {
    setGameError('');
    try { const { data } = await gameApi.get(); setGame(data); }
    catch (e) { setGameError(e.response?.data?.error || e.message); }
  };
  const handleSave = async (payload) => {
    if (editing) await habitsApi.update(editing.id, payload);
    else await habitsApi.create(payload);
    setShowForm(false);
    setEditing(null);
    await refresh();
  };
  const handleDelete = async (id) => {
    ...
    try { await habitsApi.remove(id); await refresh(); }
  ...
  const handleDragOver = (event, id) => {
    if (!draggedId || draggedId === id || query.trim()) return;
    event.preventDefault();
  ...
  const handleDrop = async (event, targetId) => {
    event.preventDefault();
    setDragOverId(null);
    if (!draggedId || draggedId === targetId || query.trim()) return;
    const from = habits.findIndex((habit) => habit.id === draggedId);
    ...
    setHabits(next);
    setDraggedId(null);
    try { await habitsApi.reorder(next.map((habit) => habit.id)); }
    catch (e) { setReorderError(e.response?.data?.error || e.message || 'Could not save habit order.'); await refresh(); }
  };
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          <div className="mini-profile" tabIndex={0} role="button">
            <div className="avatar">{(user.email?.[0] || 'U').toUpperCase()}</div>
            <div><strong>{user.email?.split('@')[0]}</strong><span>{currentStreak} day current streak</span></div>
          </div>
          <button className="ghost full" onClick={() => supabase.auth.signOut()}>Log out</button>
        </div>
      </aside>
      ...
      <main className="content">
        ...
        {view === 'today' && (
          <>
            <section className="toolbar">
              <div><p className="eyebrow">Focus</p><h2>Today</h2>{habits.length > 1 && <p className="reorder-hint">Drag habits to set your order.</p>}</div>
              <div className="toolbar-actions"><input className="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search habits…" /><button className="primary small" onClick={() => { setEditing(null); setShowForm(true); }}>＋ Add habit</button></div>
            </section>
            <section className="stats-row four">
              <div><span>Today</span><strong>{todayCompleted}<em>/{habits.length}</em></strong></div>
              <div><span>Month progress</span><strong>{progress}%</strong></div>
              <div><span>Active habits</span><strong>{habits.length}</strong></div>
              <div><span>Best current streak</span><strong>{currentStreak}<em> days</em></strong></div>
            </section>
            <section className="habit-list">
              {loading ? <div className="empty">Loading habits…</div> : filtered.length === 0 ? <div className="empty"><strong>No habits found.</strong><span>Create a habit to start building your routine.</span></div> : filtered.map((habit) => {
        ...
        {view === 'calendar' && (
          <section className="calendar-section">
            ...
            <div className="table-card"><div className="table-scroll"><table><thead><tr><th className="sticky-col">Habit</th>{days.map((day) => <th key={day} className={getDate(day) === todayKey ? 'today-head' : ''}>{day}</th>)}</tr></thead><tbody>
              {habits.length === 0 ? <tr><td colSpan={days.length + 1} className="empty">Add a habit to see your calendar.</td></tr> : habits.map((habit) => <tr key={habit.id}><td className="sticky-col"><div className="habit-name"><span className="dot" style={{ background: habit.color }} />{habit.name}</div></td>{days.map((day) => { const date = getDate(day); const checked = !!logs[`${habit.id}-${date}`]?.completed; return <td key={date} className={`${checked ? 'done' : ''} ${date === todayKey ? 'today-cell' : ''}`} style={{ '--habit-color': habit.color }}><input type="checkbox" checked={checked} onChange={() => toggle(habit.id, date)} aria-label={`${habit.name} ${date}`} /></td>; })}</tr>)}
            </tbody></table></div></div>
          </section>
        )}
...
function Insights({ habits, logs, days, year, month, monthName }) {
  ...
  return <section className="insights-grid"><div className="insight-card large"><p className="eyebrow">{monthName}</p><h2>Your month at a glance.</h2><div className="big-number">{total}<span>completions</span></div><p className="muted light">Track your strongest habits, then make the next month a little easier to win.</p></div><div className="insight-card"><p className="eyebrow">Top habit</p><h3>{top?.habit.name || '—'}</h3><strong>{top?.actual || 0}</strong><span>completed days</span></div><div className="insight-card wide"><p className="eyebrow">Habit breakdown</p>{stats.length === 0 ? <p className="muted">No habits yet.</p> : stats.map(({ habit, actual }) => <div className="bar-row" key={habit.id}><div><span>{habit.name}</span><strong>{actual}/{days.length}</strong></div><div className="bar"><i style={{ width: `${days.length ? Math.round((actual / days.length) * 100) : 0}%`, background: habit.color }} /></div></div>)}</div></section>;
}

function Rewards({ game }) {
  const stats = game?.stats;
  const xp = stats?.total_xp || 0;
  const level = game?.level || Math.floor(xp / 100) + 1;
  const progress = xp % 100;
  return <section className="rewards"><div className="reward-hero"><div><p className="eyebrow">Momentum</p><h2>Level {level}</h2><p className="muted light">{xp} total XP • {100 - progress} XP to the next level.</p></div><div className="level-ring" style={{ background: `conic-gradient(#34d399 ${progress}%, #334155 0)` }}><span>{progress}%</span></div></div><div className="stats-row three"><div><span>Total XP</span><strong>{xp}</strong></div><div><span>Completions</span><strong>{stats?.total_completed || 0}</strong></div><div><span>Badges</span><strong>{game?.badges?.length || 0}</strong></div></div><div className="insight-card"><p className="eyebrow">Badges</p>{game?.badges?.length ? <div className="badge-grid">{game.badges.map((badge) => <div className="badge" key={badge.code}><span>★</span><div><strong>{badge.name}</strong><small>{badge.description}</small></div></div>)}</div> : <p className="muted">No badges yet. Keep a streak alive to unlock your first one.</p>}</div></section>;
}
