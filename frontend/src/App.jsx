...
function AuthScreen() {
  ...
  return <main className="auth-shell"><section className="auth-card">
    ...
    <form onSubmit={submit}>
      ...
      <label>Password<div className="password-field"><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} required minLength={6} placeholder="At least 6 characters" /><button type="button" className="password-toggle" onClick={() => setShowPassword(current => !current)} aria-label={showPassword ? 'Hide password' : 'Show password'} aria-pressed={showPassword}>{showPassword ? 'Hide' : 'Show'}</button></div></label>
...
function Dashboard({ user }) {
  ...
  const { currentStreak: usageStreak } = useUsageStreak(user.id);
  ...
  {user.user_metadata?.avatar_url ? (
    ...
    <main className="content"><header className="mobile-header"><div className="brand-row"><div className="brand-mark">HT</div><strong>HabitTracker</strong></div><button className="ghost" onClick={() => supabase.auth.signOut()}>Log out</button></header>
      ...
      {view === 'insights' && <Insights habits={habits} logs={logs} days={days} year={year} month={month} monthName={monthName} todayKey={todayKey} />}
...
function Insights({ habits, logs, days, year, month, monthName, todayKey }) {
